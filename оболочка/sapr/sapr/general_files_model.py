"""! @brief Определяет класс GeneralFilesModel - всплывающее окно для открытия промежуточных файлов через
САПР моделирования. """
##
# @file general_files_model.py
#
# @brief Определяет класс GeneralFilesModel.
#
# @section description_general_files_model Описание
# Определяет создание функционального окна приложения, предназначенного для открытия промежуточных файлов через
# САПР моделирования.
# - GeneralFilesModel
#
# @section libraries_general_files_model Библиотеки/Модули
# - PyQt5
# - subprocess
# - os
#
#
# @section todo_general_files_model TODO
# - None.
#
# @section author_general_files_model Авторы
...
class GeneralFilesModel(object):
    """! Класс GeneralFilesModel.

    Определяет создание окна приложения, в котором есть возможность открыть .step файлы через САПР моделирования.
    """

    def __init__(self):
        """! Конструктор класса GeneralFilesModel.
    ...
    def setupUi(self, Dialog):
        ...
        ## Описание виджета названия окна GeneralFilesModel.
    ...
    def retranslateUi(self, Dialog):
        ...
        self.text_name_const.setText(_translate("Dialog", "МОДЕЛИРОВАНИЕ"))
    ...
    def open_step(self, module):
        """! Открывает промежуточный файл поданного на вход модуля через САПР моделирования.

                @return  Окно с запущенным приложением САПР моделирования, открывающим заданный промежуточный файл.
                """

        ## Открытие файла preferences для нахождения нужного сапр моделирования.
        ...
        if model == "LTSPICE":
            spice = fr"\\172.18.202.140\projects\{self.project}\{module}\промежуточные_файлы\{self.project}.step"
