"""! @brief Определяет класс GeneralFilesConst - всплывающее окно для открытия промежуточных файлов через
конструкторский САПР. """
##
# @file general_files_const.py
#
# @brief Определяет класс GeneralFilesConst.
#
# @section description_general_files_const Описание
# Определяет создание функционального окна приложения, предназначенного для открытия промежуточных файлов через
# конструкторский САПР.
# - GeneralFilesConst
#
# @section libraries_general_files_const Библиотеки/Модули
# - PyQt5
# - subprocess
# - os
#
#
# @section todo_general_files_const TODO
# - None.
#
# @section author_general_files_const Авторы
...
class GeneralFilesConst(object):
    """! Класс GeneralFilesConst.

    Определяет создание окна приложения, в котором есть возможность открыть .step файлы через конструкторский САПР.
    """

    def __init__(self):
        """! Конструктор класса GeneralFilesConst.
    ...
    def setupUi(self, Dialog):
        ...
        ## Описание виджета названия окна GeneralFilesConst.
    ...
    def retranslateUi(self, Dialog):
        ...
        self.text_name_const.setText(_translate("Dialog", "КОНСТРУКЦИЯ"))
    ...
    def open_step(self, module):
        """! Открывает промежуточный файл поданного на вход модуля через конструкторский САПР.

        @return  Окно с запущенным приложением конструкторского САПР, открывающим заданный промежуточный файл.
        """
        ...
        ## Запуск конструкторского САПР
        if const == "FREECAD":
            freecad = fr"\\172.18.202.140\projects\{self.project}\{module}\промежуточные_файлы\{self.project}.step"
            if os.path.exists(freecad):
